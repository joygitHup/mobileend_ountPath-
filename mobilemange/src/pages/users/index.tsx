...
export default function UsersPage() {
  ...
  const save = useMutation({
    mutationFn: (values: Partial<User>) =>
      api(`/api/v1/admin/users/${editing!.id}`, { method: 'PATCH', body: JSON.stringify(values) }),
    onSuccess: () => {
      message.success('已更新');
      setEditing(null);
  ...
  return (
    <div>
      <PageHeader
        ...
        description={routeMeta['/users'].desc}
        extra={
      ...
      <Table
        ...
        columns={[
          ...
          },
          {
            ...
            width: 100,
            render: (_, row) => (
              <Button type="link" onClick={() => { setEditing(row); form.setFieldsValue(row); }}>
                编辑
              </Button>
            ),
          },
      ...
      >
        <Form form={form} layout="vertical" onFinish={(v) => save.mutate(v)}>
          ...
          <Form.Item name="level" label="等级"><InputNumber style={{ width: '100%' }} min={1} max={20} /></Form.Item>
          <Form.Item name="safety_score" label="安全分"><InputNumber style={{ width: '100%' }} min={0} max={100} /></Form.Item>
          <Form.Item name="verified" label="实名" valuePropName="checked"><Switch /></Form.Item>
          <Form.Item name="verified_label" label="实名标签"><Input /></Form.Item>
          <Form.Item name="banned" label="封禁" valuePropName="checked"><Switch /></Form.Item>
        </Form>
      </Modal>
    </div>
  );
